function [rmse,RUL_error,RUL_upper,RUL_lower] = Degree2_Bayes(cyc,usedcap,num_cycles,i,temp_feat,temp_type)
%UNTITLED4 Summary of this function goes here
%   Detailed explanation goes here

feature = [];
if strcmpi(temp_type,'Linear')
    feature = temp_feat; 
elseif strcmpi(temp_type,'Quadratic')
    feature = temp_feat.^2; 
elseif strcmpi(temp_type,'Squareroot')
    feature = sqrt(temp_feat);
end

A = [ones(num_cycles,1) cyc(1:num_cycles) (cyc(1:num_cycles)).^2 feature(1:num_cycles)]; %Building out the matrix of data points 
...
prediction = [ones(length(cyc(num_cycles+1:end)),1),cyc(num_cycles+1:end),...
    (cyc(num_cycles+1:end)).^2, feature(num_cycles+1:end)]*mu;
% prediction2 = [ones(length(cyc2(num_cycles+1:end)),1),cyc2(num_cycles+1:end),...
%     (cyc2(num_cycles+1:end)).^2, feature(num_cycles+1:end)]*mu;

prediction_input = [ones(length(cyc(num_cycles+1:end)),1),cyc(num_cycles+1:end)...
    ,(cyc(num_cycles+1:end)).^2, feature(num_cycles+1:end)];
...
prediction_bayes = [ones(length(cyc(num_cycles+1:end)),1),cyc(num_cycles+1:end),...
    (cyc(num_cycles+1:end)).^2, feature(num_cycles+1:end)]*mu;
...
title({['Bayesian 2nd Degree Poly Model w/ ',char(temp_type),' Temperature'];['Cell ',num2str(i),', Prediction RMSE: ',num2str(rmse),...
