function [rmse,RUL_error] = Linear_Sqrt(cyc,usedcap,num_cycles,i)
%Uses least squares to get parameters for linear model fitting. 
%   Detailed explanation goes here
A = [ones(num_cycles,1) cyc(1:num_cycles) sqrt(cyc(1:num_cycles))];
y = usedcap(1:num_cycles); 
parameters = pinv(A)*y; 
prediction = [ones(length(cyc(num_cycles+1:end)),1),cyc(num_cycles+1:end),sqrt(cyc(num_cycles+1:end))]*parameters;
...
title({'Linear + Sqrt Model';['Cell ',num2str(i),', Prediction RMSE: ',num2str(rmse),', RUL Error: ',num2str(RUL_error),'%']})
