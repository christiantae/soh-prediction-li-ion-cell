function [rmse,RUL_error,RUL_upper,RUL_lower] = Linear_Sqrt_Bayes_TempFeature(cyc,usedcap,num_cycles,i,temp_feat,temp_type)
...
A = [ones(num_cycles,1) cyc(1:num_cycles) sqrt(cyc(1:num_cycles)) feature(1:num_cycles)]; %Building out the matrix of data points 
...
prediction_input = [ones(length(cyc(num_cycles+1:end)),1),cyc(num_cycles+1:end)...
    ,sqrt(cyc(num_cycles+1:end)),feature(num_cycles+1:end)];
...
prediction_bayes = [ones(length(cyc(num_cycles+1:end)),1),cyc(num_cycles+1:end),...
    sqrt(cyc(num_cycles+1:end)),feature(num_cycles+1:end)]*mu;
...
title({['Bayesian Sqrt Model w/ ',char(temp_type),' Temperature'];['Cell ',num2str(i),', Prediction RMSE: ',num2str(rmse),...
